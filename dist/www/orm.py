...
from sqlalchemy.orm import relationship, sessionmaker, joinedload, subqueryload, outerjoin
...
class MathOralCal(Base):
	...
	@classmethod
	def getStatistic(self, startdate, enddate, orderby='desc', uid=None):
		orderByStr = 's.completed_days desc,s.error_count_sum,s.consuming_time_sum' + ' ' + orderby

		with session_scope() as session:
			result = []
			sql = '''select user.id, user.name,s.error_count_sum,
			s.consuming_time_sum, s.completed_days 
			from user left join (select  math_oral_cal.user, 
			sum(math_oral_cal.error_count) AS error_count_sum, 
			sum(math_oral_cal.consuming_time) AS consuming_time_sum, 
			count(math_oral_cal.consuming_time) AS completed_days 
			from math_oral_cal 
			WHERE math_oral_cal.cal_date >= :st AND math_oral_cal.cal_date < :et 
			GROUP BY math_oral_cal.user) as s on user.id = s.user 
			where user.role != \'TEACHER\' '''
			if uid:
				sql = sql + ' and user.id=:userid'
				sql = sql + ' order by ' + orderByStr
				stmt = text(sql)
				stmt = stmt.bindparams(st=startdate, et=enddate, userid=uid)
			else:
				sql = sql + ' order by ' + orderByStr
				stmt = text(sql)
				stmt = stmt.bindparams(st=startdate, et=enddate)
			conn = session.connection()
			cals = conn.execute(stmt).fetchall()
			...
			for cal in cals:
				...
				try:
					...
					pass
				result.append(dict(name=cal[1],error=error,time=time,days=days))
